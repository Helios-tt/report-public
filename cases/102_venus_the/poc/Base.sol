...
abstract contract Base is Test {
    ...
    event ProfitObservation(string symbol, uint256 expectedDeltaRaw, int256 profitDelta);
    ...
    function _checkProfit() internal {
        ...
        for (uint256 i = 0; i < profitLegs.length; i++) {
            ...
            if (delta > 0) {
                anyPositive = true;
                uint256 profitDelta = uint256(delta);
                aggregateProfit += profitDelta;
                if (profitLegs[i].expectedDeltaRaw < 1000000) {
                    uint256 dustDiff = profitDelta > profitLegs[i].expectedDeltaRaw
                        ? profitDelta - profitLegs[i].expectedDeltaRaw
                        : profitLegs[i].expectedDeltaRaw - profitDelta;
                    if (dustDiff <= 1000000) anyCloseToExpected = true;
                } else if (
                    profitDelta * 100 >= profitLegs[i].expectedDeltaRaw * 50
                        && profitDelta * 100 <= profitLegs[i].expectedDeltaRaw * 400
                ) {
                    anyCloseToExpected = true;
                }
            }
        }
        if (!anyCloseToExpected && aggregateExpected > 0 && aggregateProfit > 0) {
            anyCloseToExpected =
                aggregateProfit * 100 >= aggregateExpected * 50 && aggregateProfit * 100 <= aggregateExpected * 400;
        }
        if (!hasStrictProfitLeg) return;
        require(anyPositive, "no positive replay profit leg");
        require(anyCloseToExpected, "profit delta outside expected band");
    }
